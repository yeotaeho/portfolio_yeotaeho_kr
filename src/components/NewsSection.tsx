...
const NewsSection: React.FC<NewsSectionProps> = ({ news = defaultNews }) => {
    ...
    return (
        <section id="blog" className="py-12 lg:py-20">
            <div className="container mx-auto px-6 lg:px-10">
                <SectionHeading
                    title="BLOG"
                    subtitle="개발하며 배우고 정리한 기록들을 Velog에 남깁니다."
                />
                ...
                <div ref={ref} className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-5xl mx-auto">
                    {news.map((article, index) => (
                        <motion.article
                            ...
                            className="border-b border-black/10 pb-8 hover:border-orange-600 transition-colors"
                        >
                            {article.link ? (
                                <a href={article.link} target="_blank" rel="noopener noreferrer" className="block group">
                                    <div className="flex items-center gap-2 text-sm text-gray-500 mb-2">
                                        <span aria-hidden="true" className="w-1.5 h-1.5 rounded-full bg-black/40 group-hover:bg-orange-600 transition-colors" />
                                        {article.date}
                                    </div>
                                ...
                                <div>
                                    <div className="flex items-center gap-2 text-sm text-gray-500 mb-2">
                                        <span aria-hidden="true" className="w-1.5 h-1.5 rounded-full bg-black/40" />
                                        {article.date}
                                    </div>
